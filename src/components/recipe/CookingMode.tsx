...
export function CookingMode() {
  const { activeOverlay, currentRecipeId, showConfetti, clearMilestone } = useAppStore();

  // Lives in the always-mounted wrapper so the celebration still fires even
  // though `handleFinish` closes the overlay (which unmounts the content).
  useEffect(() => {
    if (showConfetti) {
      confetti({
        particleCount: 100,
  ...
  const isOpen = activeOverlay === 'cooking' && !!recipe;

  return (
    <AnimatePresence>
      {isOpen && recipe && <CookingContent key={recipe.id} recipe={recipe} />}
    </AnimatePresence>
  );
}

function CookingContent({ recipe }: { recipe: Recipe }) {
  const { closeOverlay, cookingStep, nextStep, prevStep, markCooked, milestone, startTimer } =
    useAppStore();
